...
class Allegro_Gibbs_Deep_Module(GraphModuleMixin, torch.nn.Module):
    ...
    def __init__(
        ...
        latent_attention=ScalarMLPFunction,
        latent_kwargs={},
        ...
        import inspect
        latent_attention_kwargs = {k: v for k, v in latent_kwargs.items() if k in inspect.getfullargspec(latent_attention)[0]}
        latent_attention = functools.partial(latent_attention, **latent_attention_kwargs)
        env_embed = functools.partial(env_embed, **env_embed_kwargs)

        self.latents = torch.nn.ModuleList([])
        self.attention_queries = torch.nn.ModuleList([])
        self.attention_keys = torch.nn.ModuleList([])
        self.attention_values = torch.nn.ModuleList([])
        self.n_body_mlps = torch.nn.ModuleList([])
        ...
        for layer_idx, (arg_irreps, out_irreps) in enumerate(
            ...
            if layer_idx < self.num_pre_layers + self.num_layers - 1:
                ...
                self.linears.append(torch.nn.Identity())
        ...
        out_fields = [self.latent_out_field + f"_{layer}_body" for layer in range(1, num_layers + 1)] + [self.latent_out_field + "_inf_body"]
    ...
    def forward(self, data: AtomicDataDict.Type) -> AtomicDataDict.Type:
        ...
        for layer, (latent, env_embed_mlp, env_linear, tp, linear) in enumerate(zip(
            ...
            local_env_per_edge = local_env_per_edge[active_edge_center]
            ...
            scalars = attention_scalars.reshape(
                features.shape[0], -1
            )

            # do the linear
            if layer < self.num_pre_layers + self.num_layers - 1:
        ...
        # final latents
        data[self.latent_out_field + "_inf_body"] = latents
